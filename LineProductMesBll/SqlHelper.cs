...
namespace StudentMgr
{
    public static class SqlHelper
    {
        ...
        public static bool ExecuteNonQueryResult ( string cmdText ,
            params SqlParameter [ ] parameters )
        {
            using ( SqlConnection conn = new SqlConnection ( Connstr ) )
            {
        ...
        /// <param name="SQLString"></param>
        /// <returns></returns>
        public static bool ExecuteSqlTranDic ( Dictionary<object,object> SQLString )
        {
            using ( SqlConnection conn = new SqlConnection ( Connstr ) )
            {
                bool result = false;
                ...
                using ( SqlTransaction trans = conn . BeginTransaction ( ) )
                {
                    ...
                    {
                        //循环
                        foreach ( object myDE in SQLString.Keys )
                        {
                            cmdText = myDE . ToString ( );
                            cmdParms = ( Object [ ] ) SQLString [ myDE ];
                            PrepareCommand ( cmd ,conn ,trans ,cmdText ,cmdParms );
                            int val = cmd . ExecuteNonQuery ( );
                            cmd . Parameters . Clear ( );
