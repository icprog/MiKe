...
namespace LineProductMesBll . Dao
{
    public class InjectionMoldingDao
    {
        ...
        public DataTable getTablePInfo ( )
        {
            ...
            strSql . Append ( "SELECT DISTINCT RAA001,RAB003 RAA015,DEA002,DEA003,DEA057,CONVERT(FLOAT,RAB007) RAA018,ISNULL(ART04,0) DEA050,RAA008,CONVERT(FLOAT,ISNULL(ART004,0)) ART004 FROM SGMRAA A INNER JOIN SGMRAB E ON A.RAA001=E.RAB001 INNER JOIN TPADEA B ON E.RAB003=B.DEA001 LEFT JOIN MIKART C ON A.RAA015=C.ART001 LEFT JOIN (SELECT ART001,CONVERT(FLOAT,SUM(ART004)) ART04 FROM MIKART GROUP BY ART001) D ON A.RAA015=D.ART001 WHERE DEA009 IN ('M','S') AND DEA076='0502' AND RAA020='N' AND RAA024='T'" );
        ...
        {
            StringBuilder strSql = new StringBuilder ( );
            strSql . AppendFormat ( "SELECT IJA001,IJA002,IJA003,IJA004,IJA005,IJA006,IJA007,IJA008,IJA009,IJA010,IJA011,IJA012,IJA013 FROM MIKIJA WHERE IJA001='{0}'" ,oddNum );
